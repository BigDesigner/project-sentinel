...
- **Static Syntax Non-Proof & Unverified Runtime Risk Mandate (CRITICAL — Anti-Hype Rule)**: A passing static syntax check or linter run (`php -l`, `tsc --noEmit`, `cargo check`, `flutter analyze`, `go build`) proves ONLY that files parse without syntax errors. An agent MUST NEVER cite a syntax check to claim code is "100% logic verified" or "flawless". All completion reports and `walkthrough.md` documents MUST explicitly list **Unverified Runtime Risks** (e.g. external API rate limits, file permissions, network timeouts, live DB migrations) and MUST NEVER use hype/perfection claims ("100% bug-free", "flawless", "bulletproof").
- **Integration Test Real Schema Mandate (CRITICAL — Anti-Inline Schema Rule)**: Integration tests MUST run against actual production schema migration files (`0001_initial.sql`, Prisma/D1/Django migrations), not ad-hoc inline `CREATE TABLE` strings written inside test files. Inline test schemas create false-positive test passes while hiding production FK constraints, triggers, and index failures.
- **Unprotected Route & Middleware Coverage Mandate (CRITICAL)**: Every exposed API route or endpoint handler MUST be explicitly covered by global authentication, authorization, tenant isolation, and rate-limiting middleware pipelines. Uncovered routes MUST be flagged as `🔴 UNPROTECTED ROUTE / MISSING MIDDLEWARE`.
- **Cross-Stack Type & Payload Contract Verification (CRITICAL)**: Agents MUST verify that backend API payload validation schemas (Zod, Pydantic, DTOs) match the exact payload types emitted by frontend API clients (string vs object/map, int vs string). Payload type mismatches MUST be flagged as `🔴 PAYLOAD TYPE MISMATCH`.
- **CRUD Key Identity Consistency Rule (CRITICAL — Anti-Zombie Data Rule)**: Resource key generation templates (vector chunks, cache keys, DB IDs, storage paths) MUST use a single canonical key generator function/constant. Inconsistent key patterns across create and delete handlers MUST be flagged as `🔴 CRUD KEY MISMATCH / ZOMBIE DATA RISK`.
- **LLM System Prompt Injection & Template Safety Mandate (CRITICAL)**: User-supplied inputs, database rows, or user corrections MUST NEVER be raw-concatenated into LLM System Prompts. System prompts MUST use structured prompt templates and strict input sanitization to prevent permanent prompt injection.
- **Environment Config & Production Safeguard Audit Mandate (CRITICAL)**: Deployment configuration manifests (`wrangler.toml`, `docker-compose.yml`, `helm`, `.env.production`) MUST be audited to ensure development flags (`ENVIRONMENT = "development"`), `localhost` CORS origins, debug endpoints, or mock API keys are NEVER left enabled in production configurations.
