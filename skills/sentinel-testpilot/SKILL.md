...
- **Mandatory Halt & Approval Token Directive (CRITICAL):** Present the plan (scenario → type → priority → bring-up prerequisite → mock/structural flags) and HALT execution immediately. Stop calling tools. The agent MUST NOT write test files, install dependencies, bring up an environment, or execute any terminal command until the user explicitly sends an affirmative approval message (e.g. "approved", "proceed", "confirm"). Any user message that asks a question, raises a concern, or provides feedback is NOT an approval — the agent MUST adjust the plan and halt again. Do not author or run tests, and do not install dependencies, in the same response as the plan or following unapproved user chat turns.
